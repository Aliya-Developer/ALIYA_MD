...
let handler = async (m, { conn, args, text, usedPrefix, command }) => {
  ...
  if (!chat.nsfw)
    throw `💝 ALIYA MD  does not support NSFW.\n\nTo turn it on, use: ${usedPrefix}on nsfw \n\n🤤🤤🤤🤤🤤🤤🤤🤤🤤`;

  let user = global.db.data.users[m.sender].age;
  if (user < 17)
    throw `BRO,18+ hode. 😁 Age must be 18`;

  if (!text)
    throw `ALIYA MD 🇵🇰 , What do you want to search on 💝 ALIYA MD ?\n🕸️ Usage: ${usedPrefix + command} <search>\n\nEx:Mia කලීපා 🤣  or bza can use a link \nEx: .xnxx link *`;

  m.react(rwait);

  if (text.includes('http://') || text.includes('https://')) {
    if (!text.includes('xnxx.com'))
      return m.reply(`Invalid link. Only xnxx.com, Generated 💝 ALIYA MD`);
...
🕸️ ALIYA_MD
    ...
    try {
      ...
      if (searchJson.data && searchJson.data.length > 0) {
        ...
        m.reply(` No results found for the 💝 ALIYA MD search list.`);
      }
    } catch (e) {
