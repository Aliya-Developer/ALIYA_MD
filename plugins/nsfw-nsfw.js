...
let handler = async (m, { conn, usedPrefix, command }) => {
	...
	if (!global.db.data.chats[m.chat].nsfw) throw `🇵🇰 This group doesnt support 18+ adult xxx(برے کاموں سے بچنا عبادت ہے)\n\n If you enable 🇵🇰 it by \n*${usedPrefix}enable* nsfw`
    let user = global.db.data.users[m.sender].age
    if (user < 17) throw m.reply(`🇵🇰 You need to be atleast 18 years(برے کاموں سے بچنا عبادت ہے)`) 
