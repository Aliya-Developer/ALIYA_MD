...
export async function before(m, { conn, isAdmin, isBotAdmin, isOwner, isROwner }) {
    ...
    if (!m.isGroup) {
      ...
      if (bot.pmblocker && !isOwner && !isROwner) {
        ...
        await m.reply(`*Hey bza @${m.sender.split`@`[0]}, messaging the ALIYA MD 🇵🇰─❂  bot privately is currently disabled. You have been blocked from using the ALIYA MD 🇵🇰─❂ bot.*`, false, { mentions: [m.sender] });
